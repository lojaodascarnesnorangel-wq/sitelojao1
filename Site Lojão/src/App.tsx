...
const WHATSAPP_NUMBER = "5583989024081";
...
const KITS = [
  {
    name: "KIT FITNESS (600G)",
    tag: "Foco & Saúde 💪",
    badge: "Kit Fitns (600g)",
  ...
  {
    name: "KIT CASAL (400G)",
    tag: "Praticidade a Dois 👩‍❤️‍👨",
    badge: "Kit Casal (400g)",
  ...
  {
    name: "KIT DIA-A-DIA (600G)",
    tag: "O Favorito das Famílias 🏠",
    badge: "Kit Dia-A-Dia (600g)",
  ...
  {
    name: "KIT CHURRASCO P/ 10 PESSOAS (500G)",
    tag: "Fim de Semana Perfeito 🍻",
    badge: "Kit Churrasco p/ 10 Pessoas (500g)",
    originalPrice: "175,46",
    promoPrice: "158,90",
    items: [
      "Asinha",
      "Fraldinha",
  ...
  {
    name: "KIT CHURRASCO PARA 15 PESSOAS (500G)",
    tag: "A Grande Festa do Churrasco 👑",
    badge: "Kit Churrasco para 15 Pessoas (500g)",
    originalPrice: "201,12",
    promoPrice: "182,00",
    items: [
      "Farofa Especial",
      "Asinha",
...
export default function App() {
  ...
  return (
    <div className="min-h-screen bg-brand-light font-sans text-brand-black overflow-x-hidden pb-10">
      ...
      {/* Kits Econômicos Section */}
      <section className="max-w-7xl mx-auto px-10 py-12">
        <div className="text-center mb-10">
          <span className="text-brand-red font-black text-sm uppercase tracking-widest bg-brand-red/10 px-4 py-2 rounded-full">
            Mais Economia & Praticidade 📦
          </span>
          <h2 className="text-4xl md:text-5xl font-black uppercase tracking-tighter mt-4 text-brand-black">
            Nossos Kits de Carne 🔥
          </h2>
          <p className="text-gray-600 mt-2 text-lg font-medium max-w-2xl mx-auto">
            Combos completos selecionados com carinho para cobrir suas necessidades. Economize de verdade com nossos kits premium!
          </p>
        </div>

        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8">
          {KITS.map((kit, index) => {
            const savings = (parseFloat(kit.originalPrice.replace(',', '.')) - parseFloat(kit.promoPrice.replace(',', '.'))).toFixed(2).replace('.', ',');
            return (
              <motion.div
                key={index}
                ...
                className="bg-white rounded-3xl overflow-hidden border-2 border-gray-100 flex flex-col h-full shadow-md hover:shadow-xl transition-all hover:border-brand-yellow/50 group"
              >
                {/* Header Background */}
                <div className="bg-brand-black p-6 text-white relative">
                  <div className="absolute top-4 right-4 bg-brand-yellow text-brand-black text-xs font-black uppercase tracking-widest px-3 py-1 rounded-full shadow">
                    {kit.tag}
                  </div>
                  <h3 className="font-display text-2xl font-black tracking-tight uppercase text-brand-yellow leading-tight mt-4">
                    {kit.name}
                  </h3>
                </div>
                ...
                <div className="p-6 flex-grow flex flex-col">
                  {/* Ingredients */}
                  <span className="text-xs text-gray-400 font-bold uppercase tracking-widest block mb-3">Composição do Kit:</span>
                  <ul className="space-y-2 mb-8 flex-grow">
                    {kit.items.map((item, i) => (
                      <li key={i} className="flex items-center gap-2 text-gray-700 text-sm font-semibold">
                  ...
                  {/* Pricing */}
                  <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 mb-6 flex flex-col items-center justify-center">
                    <span className="text-sm text-gray-400 font-bold line-through">
                      De R$ {kit.originalPrice}
                    </span>
                    <span className="text-3xl font-black text-brand-red uppercase tracking-tight mt-1">
                      Por R$ {kit.promoPrice}
                    </span>
                    <span className="text-xs text-wa-green font-black uppercase tracking-widest mt-1 bg-wa-green/10 px-3 py-1 rounded-full">
                      Economize R$ {savings}
                    </span>
                  </div>
                  ...
                  {/* Delivery CTA Button */}
                  <a
                    href={`${BASE_WA_URL}?text=Olá!%20Gostaria%20de%20pedir%20o%20${encodeURIComponent(kit.name)}%20por%20R$%20${kit.promoPrice}`}
                    target="_blank"
                    rel="noopener noreferrer"
                    onClick={trackWhatsAppClick}
                    className="w-full text-center py-4 bg-wa-green text-white font-black btn-3d-green rounded-xl text-base flex items-center justify-center gap-2 shadow-lg"
                  >
      ...
      <footer className="bg-brand-black text-white py-10 px-10 flex flex-col md:flex-row justify-between items-center gap-8 border-t border-white/5">
        <div className="flex items-center gap-6">
          ...
          <div>
             ...
             Atendimento no balcão e delivery | <strong className="text-brand-yellow font-black">(83) 98902-4081</strong>
          </div>
        </div>
        <a 
          href={BASE_WA_URL}
